...
TEST(ClsZlogBench, AppendPlusXtn) {
  librados::Rados cluster;
  std::string pool_name = get_temp_pool_name();
  ASSERT_EQ("", create_one_pool_pp(pool_name, cluster));
  librados::IoCtx ioctx;
  ...
  // append #1
  librados::ObjectWriteOperation *op = new librados::ObjectWriteOperation;
  zlog_bench::cls_zlog_bench_append_plus_xtn(*op, 123, 456, data);
  ret = ioctx.operate("oid", op);
  ASSERT_EQ(ret, 0);

  ret = ioctx.stat("oid", &size, NULL);
  ASSERT_EQ(ret, 0);
  ASSERT_EQ(size, 12+data.length());

  // append #2
  data.clear();
  data.append(buf, sizeof(buf));
  op = new librados::ObjectWriteOperation;
  zlog_bench::cls_zlog_bench_append_plus_xtn(*op, 123, 457, data);
  ret = ioctx.operate("oid", op);
  ASSERT_EQ(ret, 0);

  ret = ioctx.stat("oid", &size, NULL);
  ASSERT_EQ(ret, 0);
  ASSERT_EQ(size, 12*2+2*data.length());
